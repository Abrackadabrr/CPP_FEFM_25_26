...
int main() {
    ...
    deduce_type d1(b_ptr);  // int *

    const int * c_ptr = new int[10];  // (добавить сюда const)
    deduce_type d2(c_ptr);  // const int *

    auto d_ptr = c_ptr;  // const int *
    deduce_type d3(d_ptr);

    auto *e_prt = c_ptr;
    deduce_type d4(e_prt);  // const int *

    auto c1 = c_ptr[1];
    deduce_type d5(c1);  // int

    auto &c2 = c_ptr[1];
    deduce_type d6(c2);  // const int &

    auto c2_ref = c2;
    deduce_type d7(c2_ref); // const int &
}
